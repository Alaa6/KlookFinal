...
export class SearchExperienceComponent implements OnInit, AfterViewInit {
  ...
  treeFlattenerCities: any = new MatTreeFlattener(
    this._transformerCities, node => node.level, node => node.expandable, node => node.Cities);
  ...
  dataSourceCities: any = new MatTreeFlatDataSource(this.treeControl, this.treeFlattenerCities);

  dataSourceCatefories: any = new MatTreeFlatDataSource(this.treeControl, this.treeFlattenerCategories);
